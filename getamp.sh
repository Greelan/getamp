    ...
    case "$ID" in
        ubuntu)
            if [ "$VERSION_ID" == "22.04" ] || [ "$VERSION_ID" == "24.04" ]; then
                wget -qO /usr/share/keyrings/docker.asc https://download.docker.com/linux/debian/gpg
                chmod a+r /usr/share/keyrings/docker.asc
                echo \
                    "deb [arch=$(dpkg --print-architecture) signed-by=/usr/share/keyrings/docker.asc] https://download.docker.com/linux/ubuntu $(. /etc/os-release && echo "${UBUNTU_CODENAME:-$VERSION_CODENAME}") stable" \
                    | tee /etc/apt/sources.list.d/docker.list > /dev/null
                apt-get update &>> "$LOG_FILE"
                DOCKER_PACKAGES="docker-ce docker-ce-cli containerd.io docker-buildx-plugin docker-compose-plugin"
            else
                echo "Docker could not be installed automatically for your Ubuntu version. Please investigate installing it manually after setup completes. See https://docs.docker.com/engine/install/ubuntu/ for more information."
                exit 66
            fi
            ;;
        debian)
            if [ "$VERSION_ID" == "11" ] || [ "$VERSION_ID" == "12" ] || [ "$VERSION_ID" == "13" ]; then
                ...
                echo "Docker could not be installed automatically for your Debian version. Please investigate installing it manually after setup completes. See https://docs.docker.com/engine/install/debian/ for more information."
                exit 66
            fi
            ;;
        rhel)
            if [ "$VERSION_ID" == "8" ] || [ "$VERSION_ID" == "9" ] || [ "$VERSION_ID" == "10" ]; then
                if [ "$PM_COMMAND" == "yum" ]; then
                    ...
                    $PM_COMMAND config-manager --add-repo https://download.docker.com/linux/rhel/docker-ce.repo
                fi
                DOCKER_PACKAGES="docker-ce docker-ce-cli containerd.io docker-buildx-plugin docker-compose-plugin"
            else
                echo "Docker could not be installed automatically for your RHEL version. Please investigate installing it manually after setup completes. See https://docs.docker.com/engine/install/rhel/ for more information."
                exit 66
            fi
            ;;
        fedora)
            if [ "$VERSION_ID" == "41" ] || [ "$VERSION_ID" == "42" ] || [ "$VERSION_ID" == "43" ]; then
                if [ "$PM_COMMAND" == "yum" ]; then
                    $PM_COMMAND "${PM_INSTALL[@]}" yum-utils
                    yum-config-manager --add-repo https://download.docker.com/linux/rhel/docker-ce.repo
                elif [ "$PM_COMMAND" == "dnf" ]; then
                ...
                echo "Docker could not be installed automatically for your Fedora version. Please investigate installing it manually after setup completes. See https://docs.docker.com/engine/install/fedora/ for more information."
                exit 66
            fi
            ;;
        centos)
            if [ "$VERSION_ID" == "9" ] || [ "$VERSION_ID" == "10" ]; then
                ...
                echo "Docker could not be installed automatically for your CentOS version. Please investigate installing it manually after setup completes. See https://docs.docker.com/engine/install/centos/ for more information."
                exit 66
            fi
            ;;
        *)
